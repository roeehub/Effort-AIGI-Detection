...
The whack-a-mole pattern: same bug, three "fix" attempts, **none committed** to the deployment artifact:
...
3. **2026-04-29 (attempt #3, Slice 7)** — fix re-derived with `target_fake_recall_min=0.30` recall-floor design + 2 new tests + budget defaults bumped 0.02/0.05 → 0.07/0.10 + cross-ckpt summary sort tiering; the runner gets new CLI args. **Uncommitted as of today.** Memory `project_contract_policy_bug.md` says "FIXED IN WORKING TREE, uncommitted. … Pending explicit user OK to commit."

The structural failure mode: design lands as analysis output (`analysis/policy_reruns_2026-04-27/`) but does not propagate to the deployment artifact (`arena/score_teams_promotion_contract.py` on `teams-relaunch-root-2026-04-17`). Slice 6 is the slice where the pattern recurs (attempt #2); Slice 7 carries attempt #3. Until commit, every contract scorecard run uses the buggy policy and the operator must manually inspect `selected_threshold` and fall back to τ=0.5. The close criterion above is the observable that breaks the loop.

This loop is **paired with `fpr-minimization-no-budget-tau-collapse`** in [`promotion_contract_evolution`](promotion_contract_evolution.md) — that loop tracks the bug itself; this loop tracks the *commit-discipline* failure mode. They close together when the fix lands on `main`.
