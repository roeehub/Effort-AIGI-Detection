...
> **⚠ Critical-reading note.** This README is current through 2026-04-30; later updates live in [`STATE.md`](STATE.md). New agents: read [`AGENTS.md`](AGENTS.md) first. Per-session `HANDOFF_*.md` files in `docs/relaunch_handoffs/` are now a frozen archive — new findings flow into STATE.md + threads + packet retros.
>
> Endorsements below have been nuanced or refuted by post-2026-04-30 work — read each thread's "Current stance" + dated update sections; don't treat README endorsements as final. Specific known drifts:
>
> 1. `face_scale_jitter@0.50 isolated` was refuted same afternoon — face-size flip rate 43.9% on the leader vs P8A 39.4%. See [`face_size_label_leak`](threads/face_size_label_leak.md) and TIMELINE 2026-04-30 afternoon.
> 2. The `mclioexb` leader did NOT promote on the contract scorecard the same evening.
> 3. Bucket-fix-as-deployed framing in [`viso_bucket_gap`](threads/viso_bucket_gap.md) was retested 2026-05-04 evening.
> 4. Contrastive-loss inference in [`clean_teams_identity_pairing`](threads/clean_teams_identity_pairing.md) has a 2026-05-04 DEBATE block flagging the symmetric-KL pair-loss premise as refuted on E2B for viso.
> 5. Per-mode τ recall lifts in [`webcam_fpr_dominance`](threads/webcam_fpr_dominance.md) and [`calibration_vs_training_aug`](threads/calibration_vs_training_aug.md) are OFFLINE-ONLY — Teams doesn't surface capture mode at inference (`feedback_per_mode_tau_not_deployable.md`).
>
> **⚠ Quality-enhancement routing bug** affects ALL R13 packets trained pre-2026-05-05: `quality_enhancement` deeplive frames were mislabeled as `deeplive_enhanced_fake` family (~5,120 frames / ~27% of enhanced-fake training volume). Fix landed 2026-05-05. Relative comparisons between R13 ckpts remain valid (uniform contamination); absolute claims on GFPGAN-enhanced fakes need post-fix retraining to revisit. See [`quality_enhancement_strategy_misrouting`](threads/quality_enhancement_strategy_misrouting.md).
