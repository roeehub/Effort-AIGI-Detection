"""Stage dor_evening frames into the identity browser.
...
SRC_ROOT = ROOT / "analysis/dor_evening_2026-05-05"
SRC_FRAMES = SRC_ROOT / "raw/all/dor_evening"
SRC_SCORES = SRC_ROOT / "scores"
DST = ROOT / "analysis/identity_browser_2026-05-05"

URI_PREFIX = "gs://local/faces_dor/evening/"
SUITE = "dor_evening"
BUCKET = "local"
LABEL = 0  # all real
...
def main() -> int:
    ...
    manifest_path = DST / "data" / "dor_evening_manifest.csv"
    manifest_df.to_csv(manifest_path, index=False)
    print(f"  manifest: {len(manifest_df)} rows -> {manifest_path}")

    # 2. Score CSVs --------------------------------------------------------
    raw_prefix = "raw/all/dor_evening/"
    for src_name, dst_name in CKPT_MAP.items():
        ...
        out_path = DST / "data" / f"dor_evening_scores_{dst_name}.csv"
