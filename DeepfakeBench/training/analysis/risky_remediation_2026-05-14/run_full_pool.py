"""Full-data validation of blend@0.50.

Pool: all gate=pass frames in grouped_manifest_v2.csv (~4035).
Score under {orig, blend@0.50} on T5C step3500 AND P8A step5000.

Output per-suite AUC + recall@{1, 2, 5, 10}% FPR, per-identity Δscore.
"""
...
log = logging.getLogger("full_pool")
...
def score(model, paths, remediation, name):
    ...
    for images, indices in loader:
        ...
        n_batches += 1
        if n_batches % 50 == 0:
            log.info(f"    [{name}] {n_batches*8}/{len(paths)} ({time.time()-t0:.0f}s)")
...
def main():
    ...
    log.info(f"manifest gate=pass: {len(m)}; exists locally: {m['exists'].sum()}")
    pool = m[m["exists"]].reset_index(drop=True)

    # Suites and labels
    log.info("suite distribution:")
    ...
    pool.to_csv(OUT_DIR / "full_pool_t5c.csv", index=False)
    log.info(f"wrote -> {OUT_DIR / 'full_pool_t5c.csv'}")
    ...
    pool.to_csv(OUT_DIR / "full_pool_p8a.csv", index=False)
    log.info(f"wrote -> {OUT_DIR / 'full_pool_p8a.csv'}")
    pool.to_csv(OUT_DIR / "full_pool_both.csv", index=False)
