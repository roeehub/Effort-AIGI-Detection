...
def prepare_video_splits(cfg_path: str = "config.yaml"
    ...
    METHODS_BUCKET = f"gs://{cfg['gcp']['methods_bucket_name']}"
    ORIGINAL_BUCKET = f"gs://{cfg['gcp']['original_bucket_name']}"
    ...
    random.seed(SEED)
    manifest_path = Path(__file__).with_name("test_frame_manifest.json")
    ...
    if manifest_path.exists():
        ...
        for bucket in [METHODS_BUCKET, ORIGINAL_BUCKET]:
    ...
    for p in frame_paths:
        if 'df40-test-frames' in p:
            method = p.split('df40-test-frames/')[-1].split('/')[0]
    ...
    # NOT NEEDED since there is only the test dataset without splitting #
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    print("\n--- Balancing Test Set ---")
    test_videos = _balance_video_list(
        videos=videos,
    ...
    print(f"\nFinal balanced split ▶ test {len(test_videos):,}")

    # random.shuffle(train_videos)
    random.shuffle(test_videos)
    return test_videos, cfg
