...
class BackendDataWriter:
    ...
    def write_video_chunk(self, participant_frames: List[ParticipantFrame],
        ...
        for participant_frame in participant_frames:
            ...
            # [DISABLED] Nonsensical ID detection removed — sanitizer no longer returns None.
            # if participant_id is None:
            #     self.logger.warning(f"Skipping participant with nonsensical ID for video chunk: '{participant_id_raw}'")
            #     continue

            self.logger.debug(
                f"Processing participant - Raw ID: {repr(participant_id_raw)}, Sanitized ID: {participant_id}")
    ...
    def create_multipart_api_data(self, participant_frames: List[ParticipantFrame],
        ...
        for participant_frame in participant_frames:
            ...
            # [DISABLED] Nonsensical ID detection removed — sanitizer no longer returns None.
            # if participant_id is None:
            #     self.logger.warning(f"Skipping participant with nonsensical ID for API data: '{participant_id_raw}'")
            #     continue

            self.logger.debug(
                f"Processing participant for API - Raw ID: {repr(participant_id_raw)}, Sanitized ID: {participant_id}")
    ...
    def _sanitize_participant_id(self, participant_id_raw: str) -> str:
        ...
        """
        # [DISABLED] Nonsensical ID detection removed — was rejecting legitimate names.
        # if self._is_nonsensical_id(participant_id_raw):
        #     self.logger.warning(f"🚨 [IMPORTANT] REJECTING NONSENSICAL PARTICIPANT ID for data storage: '{participant_id_raw}' - "
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to sanitize participant_id '{participant_id_raw}': {e}. Using generated ID: {safe_id}")
            return safe_id

    # [DISABLED] _is_nonsensical_id — was overly aggressive (>30% special chars, >10 total,
    # or 3+ consecutive). Commented out; re-enable if needed.
    ...
    #     
    #     if is_nonsensical:
    #         self.logger.debug(f"Nonsensical ID detected: '{participant_id_raw}' - "
    ...
    def write_audio_chunk_mp3(self, mp3_data: bytes, audio_batch, metadata: Dict[str, Any]) -> str:
        """Write MP3 audio chunk to storage."""
