...
class StreamingServiceImpl(pb2_grpc.StreamingServiceServicer):
    ...
    def __init__(self):
        ...
        self.banner_simulator = BannerSimulator(banner_probability=0.15, per_person_probability=1.0)  # 15% global, 100% per-person FOR IMMEDIATE TESTING
    ...
    async def _generate_per_person_banners(self, participant_frames: list) -> list:
        ...
        print(f"[Backend] Extracted {len(sanitized_participant_ids)} participant IDs for banner generation: {sanitized_participant_ids}")
        ...
        for banner in banners:
            ...
            print(f"[Backend] Banner protobuf fields - participant_id='{banner.participant_id}' banner_type='{banner.banner_type}' scope='{banner.scope}' level={banner.level}")
