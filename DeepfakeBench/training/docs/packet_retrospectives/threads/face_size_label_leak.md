...
### 2026-05-22 update — face-pool inference drops 147/196 patches and reduces lockbox_real_fpr 19% relative on Slot A v2 step3500 (FACTS only)

**Source**: `analysis/face_pool_scorecard_2026-05-22/RESULTS_FACTS_2026-05-22.md` §3a (Slot A v2 step3500 CLS vs face pool on the standing 9-suite contract); `analysis/face_pool_canary_2026-05-22/RESULTS_FACTS_2026-05-22.md` §1 (face-pool monkey-patch method and 7×7 centered-patch face mask); commit `fad721a`.

**Direct numerical findings on Slot A v2 step3500** (same ckpt, frozen weights, only the L11 pooling op changes):

- The face-pool readout discards 147 of 196 patch tokens at L11 (everything outside the centered 7×7 subgrid of the 14×14 patch grid) before applying `visual.ln_post` + `visual.proj` to the unchanged ArcFace head.
- `lockbox_real_fpr`: CLS pool 0.0191 → face pool 0.0154 (Δ = −0.0037; relative decrease 19.4 %).
- `dev_primary_real_fpr`: CLS pool 0.0655 → face pool 0.0636 (Δ = −0.0018; relative decrease 2.7 %).
- On the 800-frame canary, the real-frame score distribution std collapses from 0.305 to 0.159 (Δ = −0.146; the real-frame median shifts upward 0.175 → 0.568 while the std halves; `analysis/face_pool_canary_2026-05-22/RESULTS_FACTS_2026-05-22.md` §5).

**Mechanism note (consistent-with claim, not causal proof)**: the face-size leak's root-cause framing (2026-04-27, lines 32–35 above) is "each method's frames come from a fixed pipeline that crops at fixed tightness." The pipeline-crop signal lives in the spatial relationship between face and frame — i.e., partially in patch positions outside the face. The 49 centered face patches retain face content; the 147 non-face patches carry background, codec, capture-pipeline, and crop-frame signal. The 19 % relative reduction in `lockbox_real_fpr` by simply discarding the 147 non-face patches is consistent with the leak channel being located in non-face patches (or in the relationship between face patches and non-face patches that the CLS attention had been using to integrate context). Direct causality requires the CPU-1 per-patch ablation saliency map planned for 2026-05-23 (per `analysis/viso_fake_signature_localization_2026-05-23/RESULTS_FACTS_2026-05-23.md`, see also the new open loop `viso-fake-signature-non-face-vs-face-localization` in `viso_bucket_gap.md`).

**What this leaves open**: the face-size leak's open loop above (`face-size-label-leak`, status open, severity high, close_criterion = Cohen's d ≤ 0.10 + flip-rate ≤ 10 %) is NOT addressed by face-pool inference. Face-pool does not change the model's score-vs-crop-tightness flip rate at the per-frame level — that probe was last run in 2026-04-30 on `mclioexb` (43.9 %) and has not been re-measured on Slot A v2 step3500 under CLS or face-pool readouts. The 19 % lockbox-FPR-relative reduction is a different quantity than the flip-rate; one is the headline real-side error rate at the contract τ, the other is per-frame stability under tightness perturbation. No new open loop opened here (the existing `face-size-label-leak` loop covers the still-pending tightness re-measurement).
