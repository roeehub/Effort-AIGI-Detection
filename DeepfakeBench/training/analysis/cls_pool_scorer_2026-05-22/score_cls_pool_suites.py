"""CLS-pool 9-suite scoring for BACKBONE-SlotAv2 / BACKBONE-T5C ckpts (2026-05-22).

Identical I/O contract to score_face_pool_suites.py — uses the same frame
manifests, emits the same videos_report.csv schema — but applies NO
monkey-patch, so the OpenCLIP wrapper's standard CLS pooling is used.
This is the correct readout for ckpts trained WITHOUT face_pool_readout
(i.e. BACKBONE arms that inherit CLS-pool from their base).

Usage:
    python analysis/cls_pool_scorer_2026-05-22/score_cls_pool_suites.py \
        --report-root analysis/phase_2_backbone_2026-05-22/reports \
        --ckpt analysis/phase_2_backbone_2026-05-22/_ckpts/<run>_step<N>.pth \
        --checkpoint-key BACKBONE_SLOTAV2_<RUN>_STEP<N> \
        --suites-root analysis/face_pool_scorecard_2026-05-22/_tmp \
        --log-dir analysis/phase_2_backbone_2026-05-22/_score_logs
"""
...
logger = logging.getLogger("cls_pool_scorer")
...
def score_frames_cls_pool(
    ...
    """Return per-frame CLS-pool fake probabilities aligned to `rows`."""
    ...
    t0 = time.time()
    n_done = 0
    if True:  # no monkey-patch (CLS pool)
...
def main() -> int:
    ...
    if log_dir is not None:
        ...
        handlers.append(logging.FileHandler(log_dir / "score_cls_pool_suites.log", mode="a"))
    logging.basicConfig(
        level=logging.INFO,
    ...
    summary_json = report_root / f"_cls_pool_scoring_summary_{args.checkpoint_key.lower()}.json"
