"""
Method-domain map for Phase 3 method-conditional GRL — canonical home.

Phase 2C audit output (2026-05-01). 12 buckets total. Installed at
data/sources/method_domain_map.py for canonical import; the original draft
lives at analysis/method_class_audit_2026-05-01/proposed_method_domain_map.py.

Replaces the existing 4-class QUALITY_DOMAIN_MAP source-keyed lookup in:
    - data/sources/combined_paired.py:66 (QUALITY_DOMAIN_MAP)
    - detectors/effort_detector.py:280  (QualityDomainHead.DOMAIN_MAP)

The new map keys on `sample.method` (with `sample.source` as fallback) so the
GRL classifier sees per-fake-method labels, not per-source labels. This
isolates the `is_deeplive_enhanced` axis Phase 1A pinpointed (cos +0.10 to
+0.14 alignment with trained P17 head, monotonic across training).
...
def lookup_method_domain_with_label(method: str, source: str, label: int) -> int:
    """
    Label-aware wrapper. Real frames (label=0) from non-paired-fake sources
    route to bucket 11 (realpool_real); reals from external go to bucket 10
    (external_vcd_real); reals from df40 stay in bucket 0 (paired real+fake).

    This is the canonical entry point that iterators in combined_paired.py
    should use when assigning `quality_domain` per sample.

    Args:
        method: sample.method string
        source: sample.source string
        label: 0 (real) or 1 (fake)

    Returns:
        Bucket index 0..11 per METHOD_DOMAIN_NAMES.
    """
    fake_bucket = lookup_method_domain(method, source)

    # Fakes always use the method-conditional bucket.
    if int(label) == 1:
        return fake_bucket

    # Reals: keep df40 (paired real+fake share a bucket) and external
    # (its own bucket) where they are. Everything else collapses into
    # the realpool bucket so the GRL classifier sees a single real-side
    # target rather than method-conditional reals (which would alias real
    # frames to fake-method buckets and break the adversarial gradient).
    if fake_bucket == 0:
        return 0  # df40 paired real+fake
    if fake_bucket == 10:
        return 10  # external_vcd_real
    return 11  # realpool_real (default for non-df40, non-external reals)
