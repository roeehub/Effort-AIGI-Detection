"""Extract per-layer CLS features for T4-λ2.0 ckpts to extend the atlas.

T4-λ2.0 (run vl9y01oq) early-stopped at step 2500 because train-eval AUC
patience hit 12 evals (λ=2.0 GRL pressure suppresses train-eval AUC even
while shortcut representation may be moving). Atlas inv_mean re-extraction
on these ckpts answers: did the more-aggressive λ produce more invariance
at the encoder L11, despite the early-stop signal?

Mirrors extract_t4_features.py.
"""
...
T4_L2_CKPT_DIR = REPO_ROOT / "analysis" / "cpu_diagnostics_2026-05-10" / "_ckpts_t4_l2"

TARGET_CKPTS: Dict[str, Path] = {
    "T4_L2_step1500": T4_L2_CKPT_DIR / "periodic_effort_20260510_step1500_auc0.9910_eer0.0369.pth",
    "T4_L2_step2500": T4_L2_CKPT_DIR / "periodic_effort_20260510_step2500_auc0.9831_eer0.1037.pth",
}
...
logger = logging.getLogger("extract-t4-l2")
